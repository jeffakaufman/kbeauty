...
Product.Config.prototype.initialize = function(config) {
	...
	for (key=0; key<attributeCollection.length; key++) {
		...
		this.settings[index].id = 'attribute'+this.config.productId;
		...
		if ($('attribute'+this.config.productId)) {
			this.settings[index].elementSELECT = $('attribute'+this.config.productId);
	...
	if (this.values) {
		this.settings.each(function(element){
			...
			if (element.elementUL) {
				...
				$('hidden-attribute-'+element.attributeId+'-'+this.config.productId).value = element.value;
			}
...
//	}
	if (element.elementUL) {
		...
		$('hidden-attribute-'+element.attributeId+'-'+this.config.productId).value = '';
		element.elementUL.select('li').each(function(elementLI){
...
Product.Config.prototype.configure = function(event){
	...
	if ('select' == element.nodeName.toLowerCase() && element.settings.elementUL) {
		...
		$('hidden-attribute-'+element.settings.config.id+'-'+this.config.productId).value = element.value;
	}
	...
	if (element.settings.elementUL && 'select' != element.nodeName.toLowerCase()) {
		...
		$('hidden-attribute-'+element.settings.config.id+'-'+this.config.productId).value = element.settings.elementUL.value;
...
Product.Config.prototype.showErrorMsgOnFirstAttribute = function() {
	this.settings.each(function(element) {
		if (element.elementUL && !element.value) {
			Validation.validate($('hidden-attribute-'+element.attributeId+'-'+this.config.productId));
			throw $break;
...
Product.Config.prototype.attributeOptionIsClickable = function(elementLI) {
	...
	if (elementLI.className.indexOf('is-disabled-option') != -1 ) {
		...
		oldValue = $('hidden-attribute-'+elementUL.attributeId+'-'+this.config.productId).value;$('hidden-attribute-'+elementUL.attributeId+'-'+this.config.productId).value = '';
		//Validation.validate($('hidden-attribute-'+elementUL.attributeId+'-'+this.config.productId));
		$('hidden-attribute-'+elementUL.attributeId+'-'+this.config.productId).value = oldValue;
		return false;
	}
	return true;
}
	...
	if(element.childSettings) {
		for(var i=0;i<element.childSettings.length;i++) {
			...
			if (element.childSettings[i].elementUL) {
				...
				$('hidden-attribute-'+element.childSettings[i].attributeId+'-'+this.config.productId).value = '';
			}
		}
	}
}
...
Product.Config.prototype.resetValidation = function(element){
	...
	do {
		...
		if ( tmpElement.elementUL ) { Validation.reset( $('hidden-attribute-'+tmpElement.elementUL.attributeId+'-'+this.config.productId) ); $('hidden-attribute-'+tmpElement.elementUL.attributeId+'-'+this.config.productId).up('dd').removeClassName('validation-failed'); }         	
