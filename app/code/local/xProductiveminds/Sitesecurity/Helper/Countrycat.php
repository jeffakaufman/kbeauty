...
class Productiveminds_Sitesecurity_Helper_Countrycat extends Mage_Core_Helper_Abstract
{
    /**
     ...
     */
    public function getCountryContinent($countryCode = 1)
    {
    	...
    	if(in_array($countryCode, $europe)) {
    		$catObj = Mage::getModel('sitesecurity/countrycat')->load(Productiveminds_Sitesecurity_Model_Security::CONTINENT_CODE_EUROPE, 'code');
    		return $catObj->getCatId();
    	} else if(in_array($countryCode, $africa)) {
    		$catObj = Mage::getModel('sitesecurity/countrycat')->load(Productiveminds_Sitesecurity_Model_Security::CONTINENT_CODE_AFRICA, 'code');
    		return $catObj->getCatId();
    	} else if(in_array($countryCode, $northAmerica)) {
    		$catObj = Mage::getModel('sitesecurity/countrycat')->load(Productiveminds_Sitesecurity_Model_Security::CONTINENT_CODE_N_AMERICA, 'code');
    		return $catObj->getCatId();
    	} else if(in_array($countryCode, $southAmerica)) {
    		$catObj = Mage::getModel('sitesecurity/countrycat')->load(Productiveminds_Sitesecurity_Model_Security::CONTINENT_CODE_S_AMERICA, 'code');
    		return $catObj->getCatId();
    	} else if(in_array($countryCode, $asia)) {
    		$catObj = Mage::getModel('sitesecurity/countrycat')->load(Productiveminds_Sitesecurity_Model_Security::CONTINENT_CODE_ASIA, 'code');
    		return $catObj->getCatId();
    	} else if(in_array($countryCode, $antarctica)) {
    		$catObj = Mage::getModel('sitesecurity/countrycat')->load(Productiveminds_Sitesecurity_Model_Security::CONTINENT_CODE_ANTARCTICA, 'code');
    		return $catObj->getCatId();
    	} else if(in_array($countryCode, $oceania)) {
    		$catObj = Mage::getModel('sitesecurity/countrycat')->load(Productiveminds_Sitesecurity_Model_Security::CONTINENT_CODE_OCEANIA, 'code');
    		return $catObj->getCatId();
    	} else {
    		// Assign country to Other, if not found
    		$catObj = Mage::getModel('sitesecurity/countrycat')->load(Productiveminds_Sitesecurity_Model_Security::CONTINENT_CODE_OTHER, 'code');
    		return $catObj->getCatId();
    	}
    }
