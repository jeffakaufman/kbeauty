<?php

class Aitoc_Aitreports_Block_Export_Edit_Tab_Configuration extends Aitoc_Aitreports_Block_Export_Edit_Tab_Abstract
{
    protected $_mediumDateFormat;
    protected $_shortDateFormat;
    protected $_defaultPathValue = 'var/smartreports';
    
    public function getProfileData()
    {
        $collection = Mage::getResourceModel('aitreports/profile_collection');
        $collection->load();
        $default = Mage::getModel('aitreports/profile')
            ->setData(array(
                'profile_id' => 0,
                'name' => Mage::helper('aitreports')->__("Add New Report Profile..."),
            ));
        $collection->addItem( $default );
        return $collection;
    }

    public function getEmailSenders()
    {
        return Mage::getModel('adminhtml/system_config_source_email_identity')->toOptionArray();
    }
    
    public function getEmailTemplates()
    {
        return Mage::getModel('adminhtml/system_config_source_email_template')
            ->setPath('aitreports_template')
            ->toOptionArray();
    }
    
    public function getOrderStatuses()
    {
        $data = Mage::getModel('adminhtml/system_config_source_order_status')->toOptionArray();
        if($data[0]['value'] == '')
        {
            $data[0] = array('value' => '', 'label' => Mage::helper('aitreports')->__('All Statuses'));
        }

        return $data;
    }
    
    public function getShortDateFormat()
    {
        if (!$this->_shortDateFormat) 
        {
            $this->_shortDateFormat = Mage::app()->getLocale()->getDateStrFormat(Mage_Core_Model_Locale::FORMAT_TYPE_SHORT);
        }

        return $this->_shortDateFormat;
    }

    public function getMediumDateFormat()
    {
        if (!$this->_mediumDateFormat) 
        {
            $this->_mediumDateFormat = Mage::app()->getLocale()->getDateStrFormat(Mage_Core_Model_Locale::FORMAT_TYPE_MEDIUM);
        }

        return $this->_mediumDateFormat;
    }
    
    public function getDateRange()
    {
        return Mage::helper('aitreports/date')->getDateRangeList();
    }
    
    public function isOrdersExported()
    {
        return Mage::getModel('aitreports/export_order')->isOrdersExported( $this->getProfile()->getId() );
    }
    
    public function getCronFrequency()
    {
        return Mage::getModel('aitreports/system_config_source_cron')->toOptionArray();
    }
    
    public function getPathValue()
    {
        if($this->getProfileId()>0) {
            return $this->getValue('file/path');
        }
        return $this->_defaultPathValue;       
    }
    
    protected function _prepareForm()
    {
        $form = new Varien_Data_Form();
        
        $this->setForm($form);
        
        $this->setTemplate('aitreports/export.phtml');
        
        return parent::_prepareForm();
    }

    public function isXslExists()
    {
        return (boolean)$this->getProfile()->getXsl();
    }
    
    public function getStoreViews()
    {
        return Mage::getSingleton('adminhtml/system_store')->getStoreValuesForForm(false, true);
    }

}
