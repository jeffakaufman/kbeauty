...
class Valkyrie_DailyFierce_Adminhtml_DailyFierceController extends Mage_Adminhtml_Controller_Action
{
//  const SLIDER_DATA_IMAGES_DIR = 'sliderdata_images';
  ...
  public function indexAction()
  {
    ...
    $this->loadLayout();
      ...
      $this->_setActiveMenu('daily_fierce');

      $contentBlock = $this->getLayout()->createBlock('dailyfierce/adminhtml_dailyfierce');
  ...
  public function editAction()
  {
      ...
      $model = Mage::getModel('dailyfierce/dailyfierce');
//var_dump($model);die();
    if ($id) {
      $model->load((int) $id);
      if ($model->getId()) {
        ...
        Mage::getSingleton('adminhtml/session')->addError(Mage::helper('dailyfierce')->__('Daily Fierce does not exist'));
        $this->_redirect('*/*/');
      }
    }
    Mage::register('dailyfierce', $model);

    $this->loadLayout();

      $this->_setActiveMenu('daily_fierce');

      $contentBlock = $this->getLayout()->createBlock('dailyfierce/adminhtml_dailyfierce_edit');
      $this->_addContent($contentBlock);
...
//var_dump($data);die();
      ...
      try {
        ...
        if (!$model->getId()) {
          Mage::throwException(Mage::helper('dailyfierce')->__('Error saving fierce'));
        }

        Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('dailyfierce')->__('Fierce was successfully saved.'));
    ...
    Mage::getSingleton('adminhtml/session')->addError(Mage::helper('dailyfierce')->__('No data found to save'));
    $this->_redirect('*/*/');
  }
  ...
  public function deleteAction()
  {
    if ($id = $this->getRequest()->getParam('id')) {
      try {
        ...
        $model->setId($id);
        $model->delete();
        Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('dailyfierce')->__('The fierce has been deleted.'));
        $this->_redirect('*/*/');
        return;
      }
      catch (Exception $e) {
    ...
    Mage::getSingleton('adminhtml/session')->addError(Mage::helper('adminhtml')->__('Unable to find the fierce to delete.'));
    $this->_redirect('*/*/');
  }
