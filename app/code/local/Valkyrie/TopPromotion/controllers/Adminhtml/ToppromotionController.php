...
class Valkyrie_TopPromotion_Adminhtml_TopPromotionController extends Mage_Adminhtml_Controller_Action
{
//  const SLIDER_DATA_IMAGES_DIR = 'sliderdata_images';
  ...
  public function indexAction()
  {
    ...
    $this->loadLayout();
      ...
      $this->_setActiveMenu('top_promotion');

      $contentBlock = $this->getLayout()->createBlock('toppromotion/adminhtml_toppromotion');
  ...
  public function editAction()
  {
      ...
      $model = Mage::getModel('toppromotion/toppromotion');
//var_dump($model);die();
    if ($id) {
      $model->load((int) $id);
      if ($model->getId()) {
        ...
        Mage::getSingleton('adminhtml/session')->addError(Mage::helper('toppromotion')->__('Promotion does not exist'));
        $this->_redirect('*/*/');
      }
    }
    Mage::register('toppromotion', $model);

    $this->loadLayout();

      $this->_setActiveMenu('top_promotion');

      $contentBlock = $this->getLayout()->createBlock('toppromotion/adminhtml_toppromotion_edit');
      $this->_addContent($contentBlock);
...
//var_dump($data);die();
      ...
      try {
        ...
        if (!$model->getId()) {
          Mage::throwException(Mage::helper('toppromotion')->__('Error saving slide'));
        }

        Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('toppromotion')->__('Slide was successfully saved.'));
    ...
    Mage::getSingleton('adminhtml/session')->addError(Mage::helper('toppromotion')->__('No data found to save'));
    $this->_redirect('*/*/');
  }
  ...
  public function deleteAction()
  {
    if ($id = $this->getRequest()->getParam('id')) {
      try {
        ...
        $model->setId($id);
        $model->delete();
        Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('toppromotion')->__('The slide has been deleted.'));
        $this->_redirect('*/*/');
        return;
      }
      catch (Exception $e) {
